...
[,
 
 {
    ...
    'init' : 1,
    'url' : 'http://www.edgeofexistence.org',
    'text' : 'Link to EDGE programme website',
    'name' : 'EDGE',
 
    'logo' : 'logos/EDGE_logo.png',
 
    'leaf_link_priority': 6,
...
var linkSet = {
    ...
    [
     ...
     [-1,true,"Discover Life","http://www.discoverlife.org/mp/20q?search=||genus||+||species||&burl=www.onezoom.org&btxt=Back+to+OneZoom&bi=www.onezoom.org/logos/OZ_logo.png"],
      ...
      [-1,true,"Map of Life","http://species.mol.org/info/||genus||_||species||"],
     [-1,true,"ARKive","http://www.arkive.org/explore/species?q=||genus||%20||species||"],
     [13,true,"EDGE","http://www.edgeofexistence.org/birds/species_info.php?id=||Species_ID||"] // todo
     ],
...
[
     ...
     [2,"EDGE of Existance data version"],
 ...
 [1,],
  [2,"Thank you to the scientists who provided the EDGE data"],
 [2,"Walter Jetz, Gavin Thomas, Jeff Joy,"],
 [2,"David Redding, Klaas Hartmann and Arne Mooers"],
 [1,],
 [2,"Thank you to the EDGE team at ZSL"],
 [2,"Jamie McCallum , Carly Waterman , Nisha Owen,"],
 [2,"Jack Stewart and Alasdair Davies"],
 ...
 [1.8,"Jetz W, Thomas GH, Joy JB, Redding, Hartmann K, Mooers AO"],
 [1.8,"Global Distribution and Conservation of Evolutionary Distinctness in Birds"],
 [1.8,"Current Biology 24: 1–12 (2014)"],
 [1.8,"http://dx.doi.org/10.1016/j.cub.2014.03.011"],
 [1.8,"Also see www.birdtree.org"],
 [1,],
...
midnode.prototype.leafcolor1 = function(){
	...
	if (colourtype == 3){
        ...
        if ((metadata.leaf_meta[this.metacode][9]>4.874)&&(!this.child1))
        {
            return redlistcolor("CR");
        }
        else
        {
            return (this.branchcolor());
...
midnode.prototype.branchcolor = function(){
    ...
    else if (colourtype == 3)
    {
        ...
        colortoreturn = this.EDGE_color();
    }else if (colourtype == 5) {
        if (this.maxEDGE>= 4.874){
            colortoreturn =  brown_branch_def;
        }
        else
        {
            colortoreturn =  leaf_fill_def;
        }
...
midnode.prototype.EDGE_color = function()
{
    var EDGE_pos = (1-((fulltree.maxEDGE-this.meanEDGE)/(fulltree.maxEDGE-fulltree.minEDGE)));
    
    var colortoreturn;
    if ((this.meanEDGE)&&(this.meanEDGE > 0))
    {
        if (EDGE_pos < 0.5)
        {
            colortoreturn = 'rgb('+((Math.round(250.0*EDGE_pos)).toString())+','+(Math.round(125.0).toString())+',0)';
        }
        else
        {
            colortoreturn = 'rgb('+((Math.round(125.0)).toString())+','+(Math.round(125+250.0*(0.5-EDGE_pos)).toString())+',0)';
        }
    }
    else
    {
...
{
    if (metadata.leaf_meta[this.metacode][4] && (metadata.leaf_meta[this.metacode][4] != ""))
    {
        return conconvert(metadata.leaf_meta[this.metacode][4]);
    }
    else
    {
        return ("Not Evaluated");
...
midnode.prototype.poptxt = function() // returns text for redlist status
{
    if (metadata.leaf_meta[this.metacode][2])
    {
        switch(metadata.leaf_meta[this.metacode][2])
        {
            ...
            case "U":
            {
                if ((metadata.leaf_meta[this.metacode][4] == "EX")||(metadata.leaf_meta[this.metacode][4] == "EW"))
                {
                    return ("extinct");
                }
                else
...
midnode.prototype.dotraitsearch = function(stringin)
{
    ...
    if ((((stringin == "EX")||(stringin == "EW"))||(((stringin == "EN")||(stringin == "CR"))||((stringin == "VU")||(stringin == "NT"))))||(((stringin == "DD")||(stringin == "LC"))||(stringin == "NE")))
    {
        if (!(this.child1))
        {
            if ((metadata.leaf_meta[this.metacode][4])&&(metadata.leaf_meta[this.metacode][4] == stringin))
            {
                toret = 1;
            }
        }
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if ((stringin.toLowerCase() == "threatened")&&((metadata.leaf_meta[this.metacode][4])&&(((metadata.leaf_meta[this.metacode][4] == "CR")||(metadata.leaf_meta[this.metacode][4] == "EN"))||(metadata.leaf_meta[this.metacode][4] == "VU"))))
                    {
                        toret = 1;
                    } else if ((!(this.child1))&&((stringin == "EDGE")&&((metadata.leaf_meta[this.metacode][12])&&(metadata.leaf_meta[this.metacode][12] <= 100))))
                    {
                        toret = 1;
                    }
                }
...
midnode.prototype.traitprecalc = function()
{
	...
	if (this.child1)
	{
        ...
        this.meanEDGE = (   ((this.child1).meanEDGE * (this.child1).richness_val ) + ((this.child2).meanEDGE * (this.child2).richness_val ) )/this.richness_val;
        this.minEDGE = Math.min((this.child1).minEDGE,(this.child2).minEDGE);
        this.maxEDGE = Math.max((this.child1).maxEDGE,(this.child2).maxEDGE);
	}
	else
	{
		this.num_EX = 0;
        ...
        this.meanEDGE = metadata.leaf_meta[this.metacode][9];
        this.minEDGE = metadata.leaf_meta[this.metacode][9];
        this.maxEDGE = metadata.leaf_meta[this.metacode][9];
		
		if (metadata.leaf_meta[this.metacode][4])
		{
			switch(metadata.leaf_meta[this.metacode][4])
			{
				case "EX":
				{
					this.num_EX = 1;
...
midnode.prototype.drawLeafTextRough = function(x,y,r){
    ...
    else
    {
        ...
        if ((metadata.leaf_meta[this.metacode][12])&&(metadata.leaf_meta[this.metacode][12] <= 100))
        {
            drawArc(x,y+r*0.75,r*0.24,0,2*Math.PI,true,'rgb(255,255,255)');
            drawArc(x,y+r*0.75,r*0.2,0,2*Math.PI,true,'rgb(0,0,0)');
            context.fillStyle = this.leafcolor3();
            autotext(false,metadata.leaf_meta[this.metacode][12] , x , y + r * 0.75 , r * 0.38 , r * 0.3);
        }
...
midnode.prototype.drawLeafTextDetail = function(x,y,r){
    ...
    if (metadata.leaf_meta[this.metacode][0]){
        if (commonlabels){
            if (this.name2){
                autotext(true,this.name2 + " " + this.name1, x , y - r * 0.45 , r , r * 0.17);
            }
            autotext2(false,metadata.leaf_meta[this.metacode][0], x , y -r*0.1, r * 1.36 , r * 0.26);
        }
        else{
            if (this.name2){
                autotext2(true,this.name2 + " " + this.name1 , x , y - r * 0.1 , r * 1.36 , r * 0.26);
            }
            autotext(false,metadata.leaf_meta[this.metacode][0] , x , y - r * 0.45 , r , r * 0.1);
        }
    }
    else{
        autotext(false,"No common name", x , y - r * 0.42 , r * 0.97 , r * 0.17);
        if (this.name2){
            autotext2(true,this.name2 + " " + this.name1 , x , y , r * 1.56 , r * 0.26);
        }
        else{
            autotext2(true,this.name1, x , y , r * 1.59 , r * 0.26);
        }
    }
        
        var EDGE_uncert = Math.max(metadata.leaf_meta[this.metacode][9]-metadata.leaf_meta[this.metacode][10],metadata.leaf_meta[this.metacode][11]-metadata.leaf_meta[this.metacode][9]);
        var ED_uncert = Math.max(metadata.leaf_meta[this.metacode][6]-metadata.leaf_meta[this.metacode][7],metadata.leaf_meta[this.metacode][8]-metadata.leaf_meta[this.metacode][6]);
        
        
        autotext(false,"IUCN conservation status: " + this.extxt() , x , y + r * 0.3 , r * 1.3 , r * 0.07);
        autotext(false,"Evolutionary Distinctiveness: " +  Math.floor(metadata.leaf_meta[this.metacode][6]*100+0.5)/100 + "  (± " + Math.floor(ED_uncert*100+0.5)/100 +")", x , y + r * 0.4 , r * 1.3 , r * 0.07);
        autotext(false,"EDGE score: " + Math.floor(metadata.leaf_meta[this.metacode][9]*100+0.5)/100 + "  (± " + Math.floor(EDGE_uncert*100+0.5)/100 +")", x , y + r * 0.5 , r * 1.3 , r * 0.07);

        if (metadata.leaf_meta[this.metacode][12])
        {
            autotext(false,"EDGE rank: " + metadata.leaf_meta[this.metacode][12] , x , y + r * 0.6 , r * 1.17 , r * 0.07);
        
            if (metadata.leaf_meta[this.metacode][12] <= 100)
            {
                drawArc(x,y+r*0.82,r*0.12,0,2*Math.PI,true,'rgb(255,255,255)');
                drawArc(x,y+r*0.82,r*0.1,0,2*Math.PI,true,'rgb(0,0,0)');
                context.fillStyle = this.leafcolor3();
                autotext(false,metadata.leaf_meta[this.metacode][12] , x , y + r * 0.82 , r * 0.18 , r * 0.1);
            }
        }
        else
        {
            autotext(false,"EDGE rank > 1000 ", x , y + r * 0.6 , r * 1.17 , r * 0.07);
        }
