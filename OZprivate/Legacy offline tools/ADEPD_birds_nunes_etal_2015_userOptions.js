...
[
 ...
 [2,"To accompany the paper by Nunes, Turvey and Rosindell (2015)"],
 ...
 [3,"Data sources"],
 [1,],
 [2.2,"ADEPD conservation ranking data"],
 [1.8,"The Price of Conserving Avian Phylogenetic Diversity: A Global Prioritisation Approach"],
 [1.8,"Nunes LA, Turvey ST, Rosindell J. Philosophical Transactions of the Royal Society B (2015)"],
 [1,],
...
midnode.prototype.leafcolor1 = function(){
    ...
    if (colourtype == 3){
        ...
        if ((this.hasADEPD>0)&&(!this.child1))
        {
...
midnode.prototype.extxt_DL = function() // returns text for redlist status after downlisting
{
    if (metadata.leaf_meta[this.metacode][1] && (metadata.leaf_meta[this.metacode][1] != ""))
    {
        var temp_IUCN = metadata.leaf_meta[this.metacode][1];
        if (temp_IUCN == "CR")
        {
            temp_IUCN = "EN";
        } else if (temp_IUCN == "EN")
        {
            temp_IUCN = "VU";
        } else if (temp_IUCN == "VU")
        {
            temp_IUCN = "NT";
        } else if (temp_IUCN == "NT")
        {
            temp_IUCN = "LC";
        }
        
        return conconvert(temp_IUCN);
    }
    else
    {
        return ("Not Evaluated");
...
midnode.prototype.traitprecalc = function()
{
	...
	if (this.child1)
	{
        ...
        this.hasADEPD = Math.max((this.child1).hasADEPD,(this.child2).hasADEPD);
		
	}
	else
	{
        
        if (metadata.leaf_meta[this.metacode][8])
        {
            this.hasADEPD = 1;
        }
...
midnode.prototype.drawLeafTextDetail = function(x,y,r){
    ...
    if (!draft_only)
    {
        if (this.hasADEPD > 0)
        {
            this.drawLeafTextDetailADEPD(x,y,r)
        }
        else
        {
            // draw the text in a circle of radius r in position (x,y)
...
spend_to_string = function(x)
{
    ...
    {
        return ("$"+ (x/1000000).toString() + " million");
    }
}

midnode.prototype.drawLeafTextDetailADEPD = function(x,y,r){
    ...
    if (metadata.leaf_meta[this.metacode][0]){
        if (commonlabels){
            if (this.name2){
                autotext(true,this.name2 + " " + this.name1, x , y - r * 0.5 , r * 0.974 , r * 0.12);
            } else {
                autotext(true,this.name1, x , y - r * 0.5 , r * 0.974 ,r * 0.12);
            }
            autotext2(false,metadata.leaf_meta[this.metacode][0], x , y - r * 0.2, r * 1.36 , r * 0.2);
        }
        else{
            if (this.name2){
                autotext(true,this.name2 + " " + this.name1 , x , y - r * 0.1 , r * 1.36 , r * 0.2);
            }
            else{
                autotext(true,this.name1 , x , y - r * 0.1 , r * 1.36 , r * 0.2);
            }
            autotext2(false,metadata.leaf_meta[this.metacode][0] , x , y - r * 0.4 , r * 1.071 , r * 0.12);
        }
    }
    autotext(false,"IUCN Red List conservation status: " + this.extxt() , x , y + r * 0.1 , r * 1.8 , r * 0.08);
    autotext(false,"Estimated cost of improving status to " + this.extxt_DL() + ": " +  spend_to_string(metadata.leaf_meta[this.metacode][9]), x , y + r * 0.25 , r * 1.7 , r * 0.055);
    autotext(false,"Expected saving in evolutionary biodiversity from this: " + metadata.leaf_meta[this.metacode][5] + " million years" , x , y + r * 0.35 , r * 1.7 , r * 0.055);
    autotext(false,"Years of evolutionary biodiversity protected per dollar spent: " + metadata.leaf_meta[this.metacode][6] + " ± " + metadata.leaf_meta[this.metacode][7], x , y + r * 0.45 , r * 1.7 , r * 0.055);
    autotext(false,"Current spending on conservartion per decade: " + spend_to_string(metadata.leaf_meta[this.metacode][10]), x , y + r * 0.55 , r * 1.7 , r * 0.055);
    autotext(false,"ADEPD-cost ranking: " + metadata.leaf_meta[this.metacode][8] + " of 206" , x , y + r * 0.7 , r * 1.3 , r * 0.08);
