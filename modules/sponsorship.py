...
import ozmail
import usernames
from OZfunc import child_leaf_query, nice_name_from_otts
...
def sponsorship_config():
    ...
    try:
        out["reservation_time_limit"] = (
            float(myconf.take("sponsorship.reservation_time_limit_mins")) * 60.0
        )
    except:
        out["reservation_time_limit"] = 360.0  # seconds
    try:
        out["unpaid_time_limit"] = (
            float(myconf.take("sponsorship.unpaid_time_limit_mins")) * 60.0
        )
    except:
        out["unpaid_time_limit"] = 2.0 * 24.0 * 60.0 * 60.0  # seconds
    try:
        out["slow_payment_time_limit"] = (
            float(myconf.take("sponsorship.slow_payment_limit_mins")) * 60.0
        )
    except:
...
def sponsorship_expiry_soon_date(exp_type="soon"):
    ...
    return request.now + datetime.timedelta(
        days=sponsorship_config()["expiry_%s_days" % exp_type]
    )
...
def reservation_validate_basket_fields(basket_fields):
    ...
    if "user_sponsor_name" in basket_fields:
        if not (basket_fields["user_sponsor_name"] or ""):
            ...
            errors["user_sponsor_name"] = T("Text too long: max %s characters") % (
                max_chars,
            )

        if (
            "user_donor_name" in basket_fields
            and basket_fields.get("user_sponsor_kind") == "by"
            and not basket_fields.get("user_donor_name")
        ):
            basket_fields["user_donor_name"] = basket_fields["user_sponsor_name"]

    if (
        "user_more_info" in basket_fields
        and len(basket_fields["user_more_info"] or "") > max_chars
    ):
        errors["user_more_info"] = T("Text too long: max %s characters") % (max_chars,)

    if "user_sponsor_kind" in basket_fields and basket_fields[
        "user_sponsor_kind"
    ] not in ("by", "for"):
    ...
    if basket_fields.get("user_giftaid"):
        ...
        if missing_title:
            errors["user_donor_title_name"] = T(
                "We need your title to be able to claim gift aid"
            )
        if not (basket_fields.get("user_donor_name") or "").strip():
            if missing_title:
                errors["user_donor_title_name"] = T(
                    "We need your name and title to be able to claim gift aid"
                )
            else:
                errors["user_donor_title_name"] = T(
                    "We need your name to be able to claim gift aid"
                )
        if basket_fields.get("user_addr_nonuk"):
            # International resident
            if not (basket_fields.get("user_addr_internationaladdr") or "").strip():
                errors["user_addr_internationaladdr"] = T(
                    "We need your address to be able to claim gift aid"
                )
            # NB: We store the international addr in the house DB field
            basket_fields["user_addr_house"] = basket_fields.get(
                "user_addr_internationaladdr"
            )
            basket_fields["user_addr_postcode"] = None
        else:
            # UK resident
            if not (basket_fields.get("user_addr_house") or "").strip():
                errors["user_addr_house"] = T(
                    "We need your house number to be able to claim gift aid"
                )
            if not (basket_fields.get("user_addr_postcode") or "").strip():
                errors["user_addr_postcode"] = T(
                    "We need your post code to be able to claim gift aid"
                )
            else:
                try:
                    basket_fields["user_addr_postcode"] = normalise_postcode(
                        basket_fields["user_addr_postcode"]
                    )
                except ValueError:
                    errors["user_addr_postcode"] = T(
                        "Your postcode is not in a recognised format"
                    )
    return errors
...
def reservation_add_to_basket(basket_code, reservation_row, basket_fields):
    ...
    db = current.db
    if (
        not reservation_row.user_sponsor_name
        and "user_sponsor_name" not in basket_fields
    ):
        if basket_fields.get("prev_reservation_id", False):
            # Try digging it out of the previous entry
            prev_row = (
                db(db.expired_reservations.id == basket_fields["prev_reservation_id"])
                .select()
                .first()
            )
            if prev_row is None:
                raise ValueError(
                    "Cannot find prev_reservation_id %d"
                    % basket_fields["prev_reservation_id"]
                )
    ...
    reservation_row.update_record(basket_code=basket_code, **basket_fields)
...
def reservation_confirm_payment(basket_code, total_paid_pence, basket_fields):
    ...
    if (
        db(
            db.uncategorised_donation.PP_transaction_code
    ...
    for r in basket_rows:
        ...
        ott_price_pence = (
            db(db.ordered_leaves.ott == r.OTT_ID)
            .select(db.ordered_leaves.price)
            .first()
            .price
        )
        ...
        if r.verified_time is not None:  # i.e. is this an already paid for node?
            ...
            if remaining_paid_pence < ott_price_pence:
                ...
                r.update_record(
                    admin_comment=(
                        "reservation_confirm_payment: Transaction %s insufficient for extension. Paid %d\n%s"
                        % (
                            basket_fields["PP_transaction_code"],
                            total_paid_pence,
                            r.admin_comment or "",
            ...
            prev_row = (
                db(db.expired_reservations.id == reservation_expire(r)).select().first()
            )
            status, _, r, _ = get_reservation(prev_row.OTT_ID, basket_code)
            assert (
                status == "available"
            )  # We just expired the old one, this should work
            ...
            fields_to_update["sponsorship_duration_days"] = sponsorship_config()[
                "duration_days"
            ]
            fields_to_update[
                "sponsorship_ends"
            ] = prev_row.sponsorship_ends + datetime.timedelta(
                days=sponsorship_config()["duration_days"]
            )

            # Text was verified previously, so we can re-verify this entry
            fields_to_update["verified_time"] = request.now
            fields_to_update["reserve_time"] = prev_row.reserve_time  # Keep original
            fields_to_update["username"] = prev_row.username  # Hack: see issue #645
        else:
            # Fetch any previous row
            prev_row = (
                db(db.expired_reservations.id == r.prev_reservation_id).select().first()
            ...
            fields_to_update["sponsorship_ends"] = request.now + datetime.timedelta(
                days=sponsorship_config()["duration_days"]
            )
        ...
        if prev_row:
            for k in db.expired_reservations.fields:
                if (
                    db.expired_reservations[k].writable
                    and k in db.reservations.fields
                    and r[k] is None
                    and k not in fields_to_update
                ):
                    fields_to_update[k] = prev_row[k]

        # Fetch percentages of any partners
        partner_identifiers = partner_identifiers_for_reservation_name(
            fields_to_update.get("partner_name", r.partner_name)
        )
        if len(partner_identifiers) > 0:
            ...
            partner_percentages = [
                all_partners[p].percentage for p in partner_identifiers
            ]
        else:
            partner_percentages = []
        ...
        price_float = (
            None if ott_price_pence == float("inf") else (ott_price_pence / 100)
        )
        ...
        if remaining_paid_pence >= ott_price_pence:
            ...
            fields_to_update["verified_paid"] = (
                None if price_float is None else "{:.2f}".format(price_float)
            )
        else:
            ...
            fields_to_update["admin_comment"] = (
                "reservation_confirm_payment: Transaction %s insufficient for purchase. Paid %d\n%s"
                % (
                    basket_fields["PP_transaction_code"],
                    total_paid_pence,
                    r.admin_comment or "",
    ...
    if remaining_paid_pence > 0:
        ...
        fields_to_update = {
            k: basket_rows[0][
                k
            ]  # NB: Pull from first row to fill in any details previously set
...
def sponsorship_get_leaf_status(OTT_ID_Varin):
    ...
    if (
        (not leaf_entry)
        or (leaf_entry.ott is None)  # invalid if not in ordered_leaves
        or (" " not in leaf_entry.name)  # invalid if no OTT ID
    ):  # invalid if not a sp. name (no space/underscore)
                    ...
                    if startTime == None:
                        ...
                        if timesince < (reservation_time_limit):
                            ...
                            if (
                                ...
                                == reservation_row.user_registration_id
                            ):
...
def sponsorable_children_query(target_id, qtype="ott", check_reservations_table=True):
    ...
    if check_reservations_table:
        ...
        unavailable = db((db.reservations.verified_time != None))._select(
...
def sponsorable_children(target_id, qtype="ott", limit=None, in_reservations=None):
    ...
    if in_reservations is None:
        ...
        query = sponsorable_children_query(
            target_id, qtype, check_reservations_table=False
        )
...
def sponsor_hmac_key():
    ...
    try:
        ...
        raise ValueError(
            "sponsorship.hmac_key not set in appconfig.ini, we need this to generate renewal URLs"
        )
    if len(out) < 10:
        raise ValueError(
            "sponsorship.hmac_key is too short, or still set to the example value"
        )
    return out
...
def sponsorship_email_reminders(for_usernames=None):
    ...
    expiry_soon_trigger = expiry_soon_date - datetime.timedelta(
        days=sponsorship_config()["expiry_hysteresis"]
    )
    expiry_critical_date = sponsorship_expiry_soon_date("critical")
    expiry_critical_trigger = expiry_critical_date - datetime.timedelta(
        days=sponsorship_config()["expiry_hysteresis"]
    )
    ...
    for r in db(query).select(
        db.reservations.ALL,
        orderby=db.reservations.username | db.reservations.sponsorship_ends,
    ):
        if r.username != cur_username:
            ...
            out = dict(
                ...
                unsubscribe_url=sponsor_signed_url(
                    "sponsor_unsubscribe.html", r.username
                ),
            )
        ...
        if status != "":
            ...
            if (
                r.emailed_re_renewal_final is None
                and r.sponsorship_ends <= expiry_critical_trigger
            ):
            ...
            out["days_left"][r.OTT_ID] = (r.sponsorship_ends - request.now).days
            if (
                r.emailed_re_renewal_initial is None
                and r.sponsorship_ends <= expiry_soon_trigger
            ):
                # Below e-mail trigger date, send e-mail
...
def sponsor_renew_request_logic(
    user_identifier, mailer=None, reveal_private_data=False, automated=False
):
    """
    ...
    unames = (
        usernames.usernames_associated_to_email(user_identifier)
    ...
    if not reveal_private_data:
        info = (
            "If the user %s exists in our database, we will send them an email"
            % user_identifier
        )
    if len(user_reminders) == 0:
        ...
        user_reminders["nice_names"] = nice_name_from_otts(
            user_reminders["unsponsorable"]
            + user_reminders["not_yet_due"]
            + user_reminders["initial_reminders"]
            + user_reminders["final_reminders"],
            leaf_only=True,
            html=False,
            first_upper=True,
        )
        ...
        mailargs = ozmail.template_mail(
            "sponsor_renew_reminder", user_reminders, to=email
        )
...
def sponsorship_email_reminders_post(reminder_row):
    ...
    db(db.reservations.OTT_ID.belongs(reminder_row["initial_reminders"])).update(
        emailed_re_renewal_initial=request.now
    )
    db(db.reservations.OTT_ID.belongs(reminder_row["final_reminders"])).update(
        emailed_re_renewal_final=request.now
    )
