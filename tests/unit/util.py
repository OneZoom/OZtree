...
def time_travel(days=0, expire=True):
    ...
    current.request.now = current.request.time_travel_now - datetime.timedelta(
        days=days
    )
    if expire:
...
def find_unsponsored_otts(count, in_reservations=None, allow_banned=False):
    ...
    if len(otts) < count:
        raise ValueError(
            "Rows may not have associated prices set, visit /manage/SET_PRICES/"
        )
...
def find_unsponsored_ott(in_reservations=None, allow_banned=False):
    return find_unsponsored_otts(
        1, in_reservations=in_reservations, allow_banned=allow_banned
    )[0]
...
def clear_unittest_sponsors():
    ...
    db(
        ...
        | db.reservations.e_mail.endswith("@unittest.example.com")
    ).delete()
    db(
        db.expired_reservations.user_registration_id.startswith("UT::")
        | db.expired_reservations.basket_code.startswith("UT::")
        | db.expired_reservations.e_mail.endswith("@unittest.example.com")
    ).delete()
    db(
        db.uncategorised_donation.basket_code.startswith("UT::")
        | db.uncategorised_donation.e_mail.endswith("@unittest.example.com")
    ).delete()
...
def set_smtp(sender="me@example.com", autosend_email=1):
    ...
    if sender is None:
        ...
        if (
            not "server" in current.globalenv["myconf"]["smtp"]
            or current.globalenv["myconf"]["smtp"]["server"] is None
        ):
...
def purchase_reservation(
    ...
    if "user_sponsor_name" not in basket_details:
        ...
        basket_details["user_sponsor_name"] = basket_details.get(
            "e_mail", "User %s" % purchase_uuid
        )
    ...
    if payment_amount is None:
        ...
        payment_amount = (
            db(db.ordered_leaves.ott.belongs(otts)).select(sum).first()[sum]
        )
        if payment_amount is None:
    ...
    for ott in otts:
        status, _, reservation_row, _ = sponsorship.get_reservation(
            ott, form_reservation_code="UT::%s" % purchase_uuid
        )
        if status not in allowed_status:
            raise ValueError("OTT %d wasn't available: %s" % (ott, status))
        sponsorship.reservation_add_to_basket(
            basket_code, reservation_row, basket_details
        )
