...
class TestDatabaseSettings(object):
    ...
    def test_range_optimizer_max_mem_size(self):
        """
        Test that if we are using a mysql version > 5.7.9, the range_optimizer_max_mem_size
        is set to at least the default in 5.7.12.
        In versions from 5.7.9 - 5.7.11 it took a value of 1536000 which meant multiple
        warnings of the form: "Memory capacity of 1536000 bytes for 'range_optimizer_max_mem_size'
        exceeded. Range optimization was not done for this query."
        """
        ...
        with self.db['connection'].cursor() as db_cursor:
            ...
            if version.startswith("5.6"):
                unittest.TestCase.skipTest("Older MYSQL version not tested")
            self.db['connection'].commit() #need to commit here otherwise next select returns stale data
