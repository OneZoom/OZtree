...
3. Open e.g. `static/minlife.html` with a web browser of your choice (we recommend Chrome or Safari). Note that this file needs to stay within the static directory to work at all. You may also need to allow your browser to allow local files to be loaded via AJAX
(i.e. disabling some cross-origin checks). Different browsers do this in different ways: for example in Safari, you can choose "Disable cross-origin checks" from the "Developer" menu.
