...
var clicking;
	...
	function holdon(event)
	{
        ...
        fulltree.links2_w();
        fulltree.links2_e();
		if ((fulltree.linkclick||fulltree.linkclick_e)||(fulltree.linkclick_w))
		{
			mousehold = false;
			buttonhold = true;
			calculating = true;
	...
	function holdoff()
	{
        ...
        if (fulltree.linkclick_e)
		{
			// link out
			fulltree.emailgo();
		}
        if (fulltree.linkclick_w)
		{
			// link out
			fulltree.weblinkgo();
		}
...
function drawremind()
{
    ...
    function drawopen()
    {
        ...
        context.beginPath();
        context.fillStyle = 'rgba(0,0,0,0.85)';
        context.lineTo( myCanvas.width -135 , 10 );
        context.lineTo( myCanvas.width -115, 10);
        context.lineTo( myCanvas.width -125, 0 );
        context.fill();
        
        var offsetx = 28;
        context.beginPath();
        context.lineWidth = 1;
        context.lineTo( myCanvas.width -374 -18, 40 );
        context.lineTo( myCanvas.width -374-18, 10);
        ...
        context.beginPath();
        context.fillStyle = 'rgba(0,0,0,0.85)';
        context.lineTo( myCanvas.width -332 -offsetx, 10 );
        context.lineTo( myCanvas.width -312-offsetx, 10);
        context.lineTo( myCanvas.width -322-offsetx, 0 );
        context.fill();
        context.beginPath();
        context.fillStyle = 'rgba(0,0,0,0.85)';
        context.lineTo( myCanvas.width -364-offsetx , 10 );
        context.lineTo( myCanvas.width -344-offsetx, 10);
        context.lineTo( myCanvas.width -354-offsetx, 0 );
        context.fill();
        /*
        context.beginPath();
        context.lineWidth = 1;
        ...
        context.lineTo( 122, 0 );
        context.fill();
        
        context.fillStyle = 'rgb(255,255,255)';
        autotext(false, "more trees at www.onezoom.org" , 130, 25 , 250 , 14);
        */
	...
	function draw2()
	{
        ...
        if (popuptext2)
        {
			...
			context.lineTo( 245, 40 );
			context.fillStyle = 'rgba(0,0,0,0.85)';
			context.fill();
			context.fillStyle = 'rgb(255,255,255)';
			autotext(false, popuptext2 , 136, 25 , 250 , 14);
        }
...
{
    justopened = false;
    window.location.href = academics.home;
}

function Link2Dept()
{
    justopened = false;
    mywindow = window.open(academics.BISChome);
}

function Link2SFU()
{
    justopened = false;
    mywindow = window.open(academics.SFUhome);
}
...
// SFU
midnode.prototype.academic_calc = function()
{
    if (this.child1)
    {
        this.child1.academic_calc();
        this.child2.academic_calc();
        this.num_academic = (this.child1.num_academic + this.child2.num_academic);
        //this.name2 = this.name1 = (this.num_academic).toString();
    }
    else
    {
        this.num_academic = 0;
        this.weblink = null;
        this.maillink = null;
        for( var i = 0 ; i < academics.species.length ; i ++)
        {
            if((this.name2+"_"+this.name1) == academics.species[i][0])
            {
                this.academic_name = academics.species[i][1];
                this.num_academic = 1;
                for (var j = 0 ; j < academics.names.length ; j ++)
                {
                    if (this.academic_name == academics.names[j][0])
                    {
                        this.weblink = academics.names[j][1];
                        this.maillink = academics.names[j][2];
                    }
                }
            }
        }
        ...
        if (this.weblink)
        {
            this.name1 = this.weblink;
            this.name2 = this.maillink;
        }
         //*/
    }
}
...
function popup_OZsite()
{
    justopened = false;
    drawreminder = false;
    popuptext2 = "More trees at www.onezoom.org";
    draw2();
}
...
function popup_home()
{
    justopened = false;
    drawreminder = false;
    popuptext = "Back to Simon Fraser University Homepage";
    draw2();
}
...
function popup_SFU()
{
    justopened = false;
    drawreminder = false;
    popuptext2 = "Simon Fraser University";
    draw2();
}
...
function popup_Dept()
{
    justopened = false;
    drawreminder = false;
    popuptext2 = "SFU Biological Sciences";
    draw2();
}
