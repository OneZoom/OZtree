...
[ -d "/var/lib/dehydrated/acme-challenges" ] && NGINX_CHALLENGE_PATH="/var/lib/dehydrated/acme-challenges"
...
for SN in onezoom.org ${WWW_SERVER_NAME} ${WWW_IMAGES_SERVER_NAME}; do
    ...
    if [ ! -e "${NGINX_CERT_PATH}/${SN}/privkey.pem" ]; then
        openssl req -x509 -newkey rsa:4096 -sha256 -days 365 -nodes \
                -keyout "${NGINX_CERT_PATH}/${SN}/privkey-ss.pem" \
...
server {
    ...
    ssl_certificate      ${NGINX_CERT_PATH}/onezoom.org/fullchain.pem;
    ssl_certificate_key  ${NGINX_CERT_PATH}/onezoom.org/privkey.pem;
    ssl_trusted_certificate ${NGINX_CERT_PATH}/onezoom.org/fullchain.pem;
...
server {
    ...
    ssl_certificate      ${NGINX_CERT_PATH}/${WWW_SERVER_NAME}/fullchain.pem;
    ssl_certificate_key  ${NGINX_CERT_PATH}/${WWW_SERVER_NAME}/privkey.pem;
    ssl_trusted_certificate ${NGINX_CERT_PATH}/${WWW_SERVER_NAME}/fullchain.pem;
    ssl_dhparam ${NGINX_DHPARAM_PATH};
...
server {
    ...
    ssl_certificate      ${NGINX_CERT_PATH}/${WWW_IMAGES_SERVER_NAME}/fullchain.pem;
    ssl_certificate_key  ${NGINX_CERT_PATH}/${WWW_IMAGES_SERVER_NAME}/privkey.pem;
    ssl_trusted_certificate ${NGINX_CERT_PATH}/${WWW_IMAGES_SERVER_NAME}/fullchain.pem;
    ssl_dhparam ${NGINX_DHPARAM_PATH};
